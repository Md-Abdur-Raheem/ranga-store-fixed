...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    //cart section
    div.innerHTML = `<div class="single-product m-3 rounded-3 ">
      <div class = "mb-3 pt-3">
...
//function for get inner text from element
...
const updateTaxAndCharge = () => {
  ...
  if (priceConverted > 200) {
    const tax = priceConverted * .2;
    setInnerText("delivery-charge", 30);
    totalTax.innerText = tax.toFixed(2);
  }
  if (priceConverted > 400) {
    const tax = priceConverted * .3;
    setInnerText("delivery-charge", 50);
    totalTax.innerText = tax.toFixed(2);
  }
  if (priceConverted > 500) {
    const tax = priceConverted * .4;
...
//function for show specific product details in the ui
const showProductDetails = (id) => {
  const allProducts = loadProducts();
  const specificProduct = allProducts.filter(product => product.id === id);
  ...
  div.innerHTML = `
  <h5>${specificProduct[0].category}</h5>
  <h1>${specificProduct[0].title}</h1>
  <img class="product-image" src=${specificProduct[0].image}></img>
  <p><b>${specificProduct[0].description}</b></p>
  <h3>Price: $ ${specificProduct[0].price}</h3>
  <button onclick="addToCart(${specificProduct[0].price})" id="addToCart-btn" class="buy-now btn ">Add to cart</button>
  `;
  detailsContainer.appendChild(div);
}

//buy now btn functionality from my-cart
const buyNow = () => {
  ...
  if (total > 0) {
    ...
    if (res) {
      ...
      alert("We're sorry to loose you :(")
    }
  }
  else {
...
//to load products in ui when website loads
loadProducts();